...
with lib;
let
  cfg = config.virtualisation.oci-containers-unescaped;
  proxy_env = config.networking.proxy.envVars;

  defaultBackend = options.virtualisation.oci-containers-unescaped.backend.default;
      ...
      options = {
        ...
        dependsOn = mkOption {
          ...
          description = ''
            ...
            Use the same name as the attribute under `virtualisation.oci-containers-unescaped.containers`.
          '';
...
in
{
  options.virtualisation.oci-containers-unescaped = {
              ...
              [
                {
                  ...
                  message = "virtualisation.oci-containers-unescaped.containers.${name}: You can only define one of imageFile and imageStream";
                }
                {
                  assertion = cfg.backend == "docker" -> podman == null;
                  message = "virtualisation.oci-containers-unescaped.containers.${name}: Cannot set `podman` option if backend is `docker`.";
                }
                {
                  assertion =
